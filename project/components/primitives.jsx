...
const Reveal = ({ children, delay = 0, as = 'div', ...props }) => {
  const [ref, vis] = useReveal();
  const Tag = as;
  return (
    <Tag
      ref={ref}
      className={(props.className || '') + ' reveal' + (vis ? ' in' : '')}
      style={{ ...(props.style || {}), transitionDelay: delay + 'ms' }}
      {...Object.fromEntries(Object.entries(props).filter(([k]) => k !== 'className' && k !== 'style'))}
    >
      {children}
    </Tag>
  );
...
const Counter = ({ to, duration = 1800, prefix = '', suffix = '', decimals = 0, format = true }) => {
  const [ref, vis] = useReveal();
...
// Sparkline SVG
const Sparkline = ({ data, color = 'var(--accent)', width = 120, height = 32, fill = true, animated = true }) => {
  const [ref, vis] = useReveal();
  ...
  return (
    <svg ref={ref} width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ overflow: 'visible' }}>
      ...
      {fill && <path d={areaPath} fill={`url(#spark-grad-${color.replace(/[^a-z0-9]/gi,'')})`} opacity={vis ? 1 : 0} style={{ transition: 'opacity 1s ease 0.4s' }} />}
      <path
        d={path}
        fill="none"
        stroke={color}
...
// Live dot
const LiveDot = ({ color = 'var(--accent)' }) => (
...
Object.assign(window, { useReveal, Reveal, useCountUp, Counter, Sparkline, LiveDot, Tag });
